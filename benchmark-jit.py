...
if __name__ == '__main__':
    ...
    times = []
    fps = []
    # JIT-ify and benchmark
    for shape in shapes:
        print(f'Input shape: {shape} x {shape}', )
        traced_model = torch.jit.trace(model, torch.randn((1, 3, shape, shape)).to('cuda')).to('cuda')
        output = benchmark(traced_model, input_shape=(1, 3, shape, shape))

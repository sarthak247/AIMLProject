...
def style_loss(
    ...
    for content_fm, style_gm in zip(content_feature_maps, style_gram_matrices):
        x = gram_matrix(content_fm) - style_gm.half()
        x[x == float("Inf")] = 0
        loss += l2_squared(x)
    return loss
...
if __name__ == "__main__":
    ...
    with torch.cuda.device(args.gpu_device):
        ...
        for epoch in range(n_epochs):
            for count, sample in enumerate(traindata):
                optimizer.zero_grad()
                ...
                scaler.scale(losses["training loss"]).backward()
        ...
        torch.save(checkpoint, 'amp/checkpoint')
        writer.close()
